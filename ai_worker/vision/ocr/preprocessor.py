...
def assess_quality(image_bytes):
    ...
    logger.info("품질 검사 | blur=%.1f brightness=%.1f skew=%.1f size=%dx%d",
                blur_score, brightness, skew_angle, w, h)

    if w < MIN_WIDTH or h < MIN_HEIGHT:
        q = QUALITY_GUIDE["small"]
        return ImageQualityReport(
            status=ImageQualityStatus.SMALL,
            message=q["message"], guide=q["guide"],
            blur_score=blur_score, brightness=brightness, skew_angle=skew_angle,
        )
