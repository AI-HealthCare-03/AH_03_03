...
from ai_worker.cv.food.nutrition.scoring.schemas import DiseaseFoodScoreRecord, DiseaseScoreSet, FoodNutritionRow

REPO_ROOT = Path(__file__).resolve().parents[5]
NUTRITION_DIR = Path(__file__).resolve().parents[1]
DEFAULT_RAW_EXCEL_PATH = (
    REPO_ROOT / "etc" / "ai_worker" / "cv" / "food" / "nutrition" / "raw" / "food_nutrition_db.xlsx"
)
