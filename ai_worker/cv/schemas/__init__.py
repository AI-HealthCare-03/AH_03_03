"""
ai_worker/cv/schemas/__init__.py
...
class NutritionInfo(BaseModel):
    ...
    영양성분_신뢰도: float = Field(default=0.0, description="GPT 추정 신뢰도. 이미지 기반이므로 보통 0.5 이하")
    추정값_여부: bool = Field(default=True, description="항상 True. 정확한 수치는 식약처 DB 매칭 후 확정")


class FoodItem(BaseModel):
    name: str = Field(description="음식명")
    nutrient_category: str = Field(description="탄수화물|단백질|지방|식이섬유|비타민|미네랄|항산화|건강식")
    cooking_method: str | None = Field(None, description="조리법 (예: 튀김, 구이, 찜, 볶음, 생것)")
    estimated_amount: str | None = Field(
        None, description="추정 용량 (예: 200g, 1공기). 불확실하면 null → 사용자 직접 입력"
    )
    amount_requires_input: bool = Field(default=False, description="용량 추정 불가 시 True → 프론트에서 입력창 표시")
    nutrition: NutritionInfo | None = Field(None, description="추정 영양성분. 추정 불가 시 null")
