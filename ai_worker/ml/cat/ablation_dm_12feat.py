"""
당뇨유병 예측 — 현재흡연 + 과거음주_현재금주 제거 Ablation
...
MODEL_DIR  = os.path.join(BASE_DIR, 'outputs', 'ablation_dm_12feat')
...
BEST_PARAMS = dict(
    iterations            = 899,
    learning_rate         = 0.07751106059121869,
    depth                 = 4,
    l2_leaf_reg           = 4.326173555866765,
    bagging_temperature   = 0.6424222998382222,
    random_strength       = 0.017824706823968917,
    border_count          = 69,
...
print(f"\n[2] OOF 성능 (피처 {X_slim.shape[1]}개 + threshold=0.45)")
...
# ── 단계별 비교 ───────────────────────────────────────────────
print(f"\n[5] 단계별 비교 (threshold=0.45)")
print(f"    {'구분':<20} {'피처':>5} {'AUC':>8} {'Recall':>8} {'F1':>8} {'FN':>6}")
print("    " + "-" * 54)
print(f"    {'베이스라인':<20} {28:>5} {0.8055:>8.4f} {0.8644:>8.4f} {'N/A':>8} {'N/A':>6}")
print(f"    {'피처정리':<20} {14:>5} {0.8055:>8.4f} {0.8644:>8.4f} {'N/A':>8} {'N/A':>6}")
print(f"    {'튜닝+threshold':<20} {14:>5} {0.8087:>8.4f} {0.8644:>8.4f} {0.3796:>8.4f} {110:>6}")
print(f"    {'흡연/음주 제거':<20} {X_slim.shape[1]:>5} {oof_auc:>8.4f} {oof_recall:>8.4f} {oof_f1:>8.4f} {cm[1,0]:>6}")
