"""
고혈압유병 예측 — importance 낮은 피처 추가 제거 Ablation Test
Python 3.9 | catboost>=1.2 | scikit-learn>=1.4
제거: 직업 8개 + 타질환가족력 6개 + importance≈0 피처 4개
피처: 28개 → 10개
"""
...
MODEL_DIR  = os.path.join(BASE_DIR, 'outputs', 'ablation_slim_HTN')
...
print(f"[0] 제거 컬럼 ({len(remove_cols)}개): 직업 8개 + 타질환가족력 6개 + importance≈0 4개")
...
print(f"    {'관련가족력만':<20} {14:>5} {0.8577:>8.4f} {0.8530:>8.4f} {0.6458:>8.4f} {247:>6} {1325:>6}")
print(f"    {'slim (10개)':<20} {X_slim.shape[1]:>5} {oof_auc:>8.4f} {oof_recall:>8.4f} {oof_f1:>8.4f} {cm[1,0]:>6} {cm[0,1]:>6}")
