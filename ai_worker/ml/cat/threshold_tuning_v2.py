...
for disease, cfg in CONFIGS.items():
    ...
    for trial_ver, dir_key in [('50', 'dir_50'), ('100', 'dir_100')]:
        ...
        # 저장
        results_df.to_csv(
            os.path.join(OUTPUT_DIR, f'threshold_results_{disease}_{trial_ver}.csv'),
            index=False
        )
    ...
    print("  " + "-" * 68)
    print(f"  {'50 trial':<20} {b50['threshold']:>10.2f} {b50['auc']:>8.4f} {b50['recall']:>8.4f} {b50['f1']:>8.4f} {b50['fn']:>6.0f} {b50['fp']:>6.0f}")
    print(f"  {'100 trial':<20} {b100['threshold']:>10.2f} {b100['auc']:>8.4f} {b100['recall']:>8.4f} {b100['f1']:>8.4f} {b100['fn']:>6.0f} {b100['fp']:>6.0f}")
...
for disease, res in final_results.items():
    print(f"{disease:<15} {res['adopted_trial']:>6} {res['threshold']:>10.2f} "
          f"{res['auc']:>8.4f} {res['recall']:>8.4f} {res['f1']:>8.4f} {res['fn']:>6}")

# 저장
pd.DataFrame(final_results).T.to_csv(
    os.path.join(OUTPUT_DIR, 'final_threshold_summary.csv')
)
print(f"\n저장 완료 → {OUTPUT_DIR}")
