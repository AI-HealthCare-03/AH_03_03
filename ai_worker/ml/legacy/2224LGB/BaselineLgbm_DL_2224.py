"""
이상지질혈증유병 예측 베이스라인 — LightGBM (hn18~24 통합)
...
MODEL_DIR = str(Path(__file__).parent.parent.parent.parent / "ai_worker" / "ml" / "LGB18~24" / "outputs" / "baseline_lgbm_DL_2224")
...
if len(candidates) > 0:
    ...
    best_thr = best_row["threshold"]
    print(f"    ⚠️  Recall >= {RECALL_MIN} 만족 threshold 없음 → Recall 최대 사용: {best_thr:.2f}")
    print(
        f"       Recall: {best_row['recall']:.4f} | Precision: {best_row['precision']:.4f} | F1: {best_row['f1']:.4f}"
    )
