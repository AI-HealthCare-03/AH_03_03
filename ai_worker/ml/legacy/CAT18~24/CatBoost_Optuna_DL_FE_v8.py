"""
이상지질혈증유병 예측 — CatBoost + Optuna + FE v8 (hn18~24 통합)
Python 3.13 | catboost>=1.2 | optuna>=3.0 | scikit-learn>=1.4

변경사항 (v8 = v1 베이스라인 + 나이×성별 + Optuna):
- v1 피처 구성 그대로 유지 (가족력 개별+합산, 직업 더미 포함)
- 나이 × 성별 상호작용 변수 추가
- Optuna 하이퍼파라미터 튜닝 적용

FE 조합: 나이구간 + BMI구간 + 가족력합산 + BMI_X_나이 + 나이_X_성별
...
MODEL_DIR: str = str(Path(__file__).parent.parent.parent.parent / "ai_worker" / "ml" / "CAT18~24" / "outputs" / "optuna_DL_FE_v8")
