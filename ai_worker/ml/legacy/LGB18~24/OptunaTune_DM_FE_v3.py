"""
당뇨유병 예측 — Optuna 하이퍼파라미터 튜닝 + 피처 엔지니어링 (hn18~24 통합)
...
Optimization Target: OOF F1 최대화  ← [v3 변경] Precision → F1
Monitoring         : Recall, Precision, ROC-AUC 함께 기록
Trial 수           : 50
...
MODEL_DIR: str = str(Path(__file__).parent.parent.parent.parent / "ai_worker" / "ml" / "LGB18~24" / "outputs" / "optuna_DM_FE_v3")
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 3. 체중 구간화
    if USE_WEIGHT_BIN:
        wt_bins: list[float] = [0, 50, 70, 90, np.inf]
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# [v3 변경] tune_threshold: Precision 최대화 → F1 최대화
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def tune_threshold(
    ...
    Recall >= recall_min 만족하는 threshold 중 F1 최대인 것을 선택.

    Returns: (best_thr, best_f1, final_recall, final_prec)
    """
    best_f1: float = 0.0
    best_thr: float = 0.30
    ...
    for thr in THRESHOLD_RANGE:
        ...
        # [v3] Recall 조건 만족 시 F1 최대인 threshold 선택
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# [v3 변경] objective: return best_prec → return best_f1
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def objective(
    trial: Trial,
    ...
    """
    Optuna objective 함수.
    Recall >= RECALL_MIN 만족 시 OOF F1 최대화.  ← [v3 변경]
    """
    params: dict[str, Any] = {
        "objective": "binary",
        "metric": "auc",
    ...
    trial.set_user_attr("oof_precision", round(oof_prec, 4))  # [v3] precision도 기록
    ...
    # [v3] F1을 반환 → Optuna가 F1 최대화하는 파라미터 탐색
    return best_f1
...
def main() -> None:
    ...
    print(f"    OOF F1        : {best.value:.4f}")  # [v3] Precision → F1
    print(f"    OOF Recall    : {best.user_attrs.get('oof_recall', '-')}")
    print(f"    OOF Precision : {best.user_attrs.get('oof_precision', '-')}")  # [v3] 추가
