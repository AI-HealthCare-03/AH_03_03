...
for disease, cfg in CONFIGS.items():
    ...
    print("    " + "-" * 44)
    print(
        f"    {'흡연 제외':<15} {baseline['auc']:>8.4f} {baseline['recall']:>8.4f} {baseline['f1']:>8.4f} {baseline['fn']:>6}"
    )
    print(f"    {'흡연 포함':<15} {oof_auc:>8.4f} {oof_recall:>8.4f} {oof_f1:>8.4f} {cm[1, 0]:>6}")
    print(
        f"    변화           {oof_auc - baseline['auc']:>+8.4f} {oof_recall - baseline['recall']:>+8.4f} {oof_f1 - baseline['f1']:>+8.4f} {cm[1, 0] - baseline['fn']:>+6}"
    )
