"""
당뇨유병 예측 — CatBoost + Optuna 하이퍼파라미터 튜닝 + FE v4 (hn18~24 통합)
...
새로 추가된 FE 플래그:
  USE_FAMILY_X_BMI   : 당뇨가족력_합산 × BMI
  USE_FAMILY_X_AGE   : 당뇨가족력_합산 × 나이
...
MODEL_DIR: str = str(Path(__file__).parent.parent.parent.parent / "ai_worker" / "ml" / "CAT18~24" / "outputs" / "optuna_DM_FE_v4")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
USE_FAMILY_X_BMI: bool = False  # 당뇨가족력_합산 × BMI
USE_FAMILY_X_AGE: bool = True  # 당뇨가족력_합산 × 나이
USE_ALCOHOL_X_BMI: bool = False  # 음주량 × BMI
USE_ALCOHOL_COMBO: bool = False  # 음주빈도 × 음주량
USE_ACTIVITY_TOTAL: bool = False  # 근력운동일수 + 걷기일수
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── 신규 상호작용 FE ─────────────────────────────────────
    # USE_FAMILY_SUM이 True일 때만 의미 있음 (당뇨가족력_합산 존재 전제)
    if USE_FAMILY_X_BMI:
        df["가족력_X_BMI"] = df["당뇨가족력_합산"] * df["BMI"]
        added += ["가족력_X_BMI"]
        print("  [ON] 당뇨가족력_합산 × BMI")

    if USE_FAMILY_X_AGE:
        df["가족력_X_나이"] = df["당뇨가족력_합산"] * df["나이"]
        added += ["가족력_X_나이"]
        print("  [ON] 당뇨가족력_합산 × 나이")
...
def main() -> None:
    ...
    best_thr: float = float(best.user_attrs.get("best_thr", 0.43))
