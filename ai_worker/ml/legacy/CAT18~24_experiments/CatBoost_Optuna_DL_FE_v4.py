"""
이상지질혈증유병 예측 — CatBoost + Optuna 하이퍼파라미터 튜닝 + FE v4 (hn18~24 통합)
Python 3.13 | catboost>=1.2 | optuna>=3.0 | scikit-learn>=1.4

FE 조합: 나이구간 + BMI구간 + 가족력합산 + BMI_X_나이 (기존 유지)
       + 상호작용 피처 추가 (ablation 플래그로 개별 제어)

새로 추가된 FE 플래그:
  USE_FAMILY_X_BMI   : 고지혈증가족력_합산 × BMI
  USE_FAMILY_X_AGE   : 고지혈증가족력_합산 × 나이
  USE_ALCOHOL_X_BMI  : 음주량 × BMI
  USE_ALCOHOL_COMBO  : 음주빈도 × 음주량
  USE_ACTIVITY_TOTAL : 근력운동일수 + 걷기일수 (전체 활동량)
  USE_GENDER_X_AGE   : 성별 × 나이 (DL은 성별 importance 높아서 추가)
...
MODEL_DIR: str = str(Path(__file__).parent.parent.parent.parent / "ai_worker" / "ml" / "CAT18~24" / "outputs" / "optuna_DL_FE_v4")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# FE 플래그 — 기존 (v3 확정 조합, 건드리지 말 것)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
USE_AGE_BIN: bool = True  # 나이 6구간 원핫
USE_BMI_BIN: bool = True  # BMI 구간화
USE_WEIGHT_BIN: bool = False
USE_ALCOHOL_RISK: bool = False
USE_WALK_LEVEL: bool = False
USE_STRENGTH: bool = False
USE_FAMILY_SUM: bool = True  # 가족력 합산
USE_BMI_X_AGE: bool = True  # BMI × 나이
USE_OBESITY_FLAG: bool = False

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# FE 플래그 — 신규 상호작용 피처 (ablation 대상)
# 하나씩 True로 바꿔가며 실험할 것
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
USE_FAMILY_X_BMI: bool = True  # 고지혈증가족력_합산 × BMI
USE_FAMILY_X_AGE: bool = True  # 고지혈증가족력_합산 × 나이
USE_ALCOHOL_X_BMI: bool = True  # 음주량 × BMI
USE_ALCOHOL_COMBO: bool = True  # 음주빈도 × 음주량
USE_ACTIVITY_TOTAL: bool = True  # 근력운동일수 + 걷기일수
USE_GENDER_X_AGE: bool = True  # 성별 × 나이 (DL 전용 — 성별 importance 높음)
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── 신규 상호작용 FE ─────────────────────────────────────
    # DL은 고지혈증가족력_합산 기준으로 상호작용 생성
    if USE_FAMILY_X_BMI:
        df["가족력_X_BMI"] = df["고지혈증가족력_합산"] * df["BMI"]
        added += ["가족력_X_BMI"]
        print("  [ON] 고지혈증가족력_합산 × BMI")

    if USE_FAMILY_X_AGE:
        df["가족력_X_나이"] = df["고지혈증가족력_합산"] * df["나이"]
        added += ["가족력_X_나이"]
        print("  [ON] 고지혈증가족력_합산 × 나이")

    if USE_ALCOHOL_X_BMI:
        df["음주량_X_BMI"] = df["음주량"].fillna(0) * df["BMI"]
        added += ["음주량_X_BMI"]
        print("  [ON] 음주량 × BMI")

    if USE_ALCOHOL_COMBO:
        df["음주_복합"] = df["음주빈도"].fillna(0) * df["음주량"].fillna(0)
        added += ["음주_복합"]
        print("  [ON] 음주빈도 × 음주량")

    if USE_ACTIVITY_TOTAL:
        df["전체활동량"] = df["근력운동일수"].fillna(0) + df["걷기일수"].fillna(0)
        added += ["전체활동량"]
        print("  [ON] 근력운동일수 + 걷기일수 (전체활동량)")

    if USE_GENDER_X_AGE:
        df["성별_X_나이"] = df["성별"] * df["나이"]
        added += ["성별_X_나이"]
        print("  [ON] 성별 × 나이 (DL 전용)")
...
def main() -> None:
    ...
    best_thr: float = float(best.user_attrs.get("best_thr", 0.45))
