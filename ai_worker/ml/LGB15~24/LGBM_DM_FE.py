"""
고혈압유병 예측 — LightGBM + 피처 엔지니어링 (hn15~24 통합)
...
# ── 경로 설정 ─────────────────────────────────────────────────
DATA_PATH = str(Path(__file__).parent.parent.parent / "ai_worker" / "data" / "hn_all_preprocessed.csv")
MODEL_DIR = str(Path(__file__).parent.parent.parent / "ai_worker" / "ml" / "LGB15~24" / "outputs" / "baseline_lgbm_DM_FE")
