"""
이상지질혈증유병 예측 — CatBoost + Optuna + FE v4 (hn18~24 통합)
Python 3.13 | catboost>=1.2 | optuna>=3.0 | scikit-learn>=1.4

변경사항 (v4 = v3 + 피처 정리):
- 가족력 개별 변수(부/모/형제) 전부 제거 → 합산 3개만 유지
- 직업 더미 변수 전부 제거
- 나이 구간화 유지

FE 조합: 나이구간 + BMI구간 + 가족력합산(3개) + BMI_X_나이
...
MODEL_DIR: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT18~24/outputs/optuna_DL_FE_v5"
...
USE_ALCOHOL_BURDEN: bool = True  # 음주빈도 × 음주량 부담점수
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if USE_ALCOHOL_BURDEN:
        freq = pd.to_numeric(df.get("음주빈도"), errors="coerce")
        qty  = pd.to_numeric(df.get("음주량"),   errors="coerce")
        df["음주_부담"] = freq * qty
        added += ["음주_부담"]
        print("  [ON] 음주 부담점수 (빈도 × 양)")
...
def drop_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"[v5] 제거된 피처 수: {len(to_drop)}")
    return df
