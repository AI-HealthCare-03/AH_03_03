"""
이상지질혈증유병 예측 — CatBoost + Optuna + FE v6 (hn18~24 통합)
Python 3.13 | catboost>=1.2 | optuna>=3.0 | scikit-learn>=1.4

변경사항 (v6 = v4 + 가족력 이진 플래그):
- 가족력 합산(0~3) → 있음/없음 이진 플래그(0/1)로 변경
- 직업 더미 변수 전부 제거
- 음주 부담점수 제거 (v5에서 효과 없음 확인)
- 나이 구간화 유지

FE 조합: 나이구간 + BMI구간 + 가족력이진(3개) + BMI_X_나이
...
MODEL_DIR: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT18~24/outputs/optuna_DL_FE_v6"
...
def drop_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"[v6] 제거된 피처 수: {len(to_drop)}")
    return df
