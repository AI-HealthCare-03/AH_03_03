"""
이상지질혈증유병 예측 — CatBoost + FE v3 (hn18~24 통합)
Python 3.13 | catboost>=1.2 | scikit-learn>=1.4

변경사항 (v3 = v2 + 가족력이진 + 나이×성별):
- 가족력 합산(0~3) → 있음/없음 이진 플래그(0/1)로 변경
- 나이 × 성별 상호작용 변수 추가 (폐경 나이대 여성 DL 위험 포착)
- 베이스라인 파라미터 유지 (FE 효과만 순수하게 비교)

FE 조합: 나이구간 + BMI구간 + 가족력이진(3개) + BMI_X_나이 + 나이_X_성별
...
MODEL_DIR: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT18~24/outputs/catboost_DL_FE_v3"
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 가족력 개별 변수 — 합산으로 대체
DROP_FAMILY_INDIVIDUAL: list[str] = [
    "고혈압가족력_부", "고혈압가족력_모", "고혈압가족력_형제",
    "당뇨가족력_부",   "당뇨가족력_모",   "당뇨가족력_형제",
    "고지혈증가족력_부", "고지혈증가족력_모", "고지혈증가족력_형제",
]

# 직업 더미 변수 — 중요도 낮아 전체 제거
DROP_OCCUPATION: list[str] = [
    "직업_무직", "직업_사무", "직업_기능노무", "직업_서비스판매",
    "직업_주부학생", "직업_농림어업", "직업_관리전문", "직업_작업미상",
]
...
USE_FAMILY_FLAG: bool = True  # 가족력 이진 플래그 (있음/없음)
USE_BMI_X_AGE: bool = True
USE_AGE_X_SEX: bool = True    # 나이 × 성별 (폐경 나이대 여성 DL 위험 포착)
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if USE_FAMILY_FLAG:
        ...
        added += ["고혈압가족력_있음", "당뇨가족력_있음", "고지혈증가족력_있음"]
        print("  [ON] 가족력 이진 플래그 (있음/없음)")
...
def drop_features(df: pd.DataFrame) -> pd.DataFrame:
    to_drop = [c for c in DROP_FAMILY_INDIVIDUAL + DROP_OCCUPATION if c in df.columns]
    df = df.drop(columns=to_drop)
    print(f"[v3] 제거된 피처 수: {len(to_drop)}")
    print(f"     가족력 개별: {[c for c in DROP_FAMILY_INDIVIDUAL if c in df.columns or True][:9]}")
    print(f"     직업 더미:   {[c for c in DROP_OCCUPATION if c in df.columns or True]}")
    return df
...
def main() -> None:
    ...
    print(f"\n[1] Feature 수: {X.shape[1]}  (v1 대비 -{38 - X.shape[1]}개)")
