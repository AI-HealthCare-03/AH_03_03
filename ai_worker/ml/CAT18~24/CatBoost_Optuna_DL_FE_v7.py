"""
이상지질혈증유병 예측 — CatBoost + Optuna + FE v7 (hn18~24 통합)
...
MODEL_DIR: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT18~24/outputs/optuna_DL_FE_v7"
...
def drop_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"[v7] 제거된 피처 수: {len(to_drop)}")
    return df
...
def main() -> None:
    ...
    oof_proba, fold_models, scores_df = retrain_with_best_params(
        best_params, class_ratio, X_train, y_train
    )
