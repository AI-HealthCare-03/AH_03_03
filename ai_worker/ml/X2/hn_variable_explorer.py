...
def create_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def classify_htn(row):
        ...
        if s >= 160 or d >= 100: return 4   # 2단계 고혈압
        if s >= 140 or d >= 90:  return 3   # 1단계 고혈압
        if s >= 130 or d >= 80:  return 2   # 고혈압전단계
        if s >= 120:             return 1   # 주의
        return 0                             # 정상
...
CANDIDATE_GROUPS = {
    ...
    "검진_혈액": [
        ...
        "HE_WBC", "HE_RBC",       # 혈구수
        "HE_Bplt",                # 혈소판
        "HE_HB", "HE_HCT",        # 헤모글로빈/헤마토크리트
        "HE_Uacid",               # 요산
        "HE_hsCRP",               # 고감도 CRP
        "HE_Ca", "HE_P", "HE_Mg", # 전해질
    ],
    "검진_소변": [
        "HE_Upro",  # 요단백
        "HE_Uglu",  # 요당
    ...
    # ── 생활습관 ───────────────────────────────────────────────
    "생활_수면": [
        "BP16_11", "BP16_13",   # 주중 취침/기상시각(시간)
        "BP16_21", "BP16_23",   # 주말 취침/기상시각(시간)
    ],
    "생활_신체활동_고강도": [
        "BE3_71", "BE3_72",     # 고강도(일) 여부/일수
        "BE3_75", "BE3_76",     # 고강도(여가) 여부/일수
    ],
    "생활_신체활동_중강도": [
        "BE3_81", "BE3_82",     # 중강도(일)
    ...
    "생활_수면_시간_계산용": [
        ...
        "BP16_12", "BP16_14",  # 주중 취침/기상 분
        "BP16_22", "BP16_24",  # 주말 취침/기상 분
    ],
    ...
    # ── 기타 질병력 / 가족력 ───────────────────────────────────
    "가족력_추가": [
        "HE_IHDfh1", "HE_IHDfh2", "HE_IHDfh3",  # 허혈성심장질환
        "HE_STRfh1", "HE_STRfh2", "HE_STRfh3",  # 뇌졸중
    ],
    "질병력": [
        "DI3_dg",   # 뇌졸중 진단
        "DI4_dg",   # 심근경색/협심증
    ...
    for ax, (tgt_name, col) in zip(axes, TARGET_MAP.items()):
        ...
        trend = df.groupby("year")[col].apply(
            lambda x: (pd.to_numeric(x, errors="coerce") == 1).sum()
                      / pd.to_numeric(x, errors="coerce").notna().sum() * 100
        )
