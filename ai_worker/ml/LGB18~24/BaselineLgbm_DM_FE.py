"""
당뇨유병 예측 — LightGBM + 피처 엔지니어링 (hn18~24 통합)
...
MODEL_DIR = '/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/LGB18~24/outputs/baseline_lgbm_DM_FE'
...
USE_BMI_BIN      = True   # BMI 구간화 (한국인 기준: 0~4)
USE_WEIGHT_BIN   = False   # 체중 구간화
USE_ALCOHOL_RISK = False   # 음주위험군 (비음주/저위험/고위험)
USE_WALK_LEVEL   = False   # 걷기 활동량
USE_STRENGTH     = False   # 근력운동 활동량
USE_FAMILY_SUM   = False   # 가족력 합산 스코어
...
if USE_BMI_BIN:
    ...
    added += ['BMI_구간']
    print(f"  ✅ BMI 구간화: 0=정상/1=과체중/2=비만1/3=비만2")
...
if USE_ALCOHOL_RISK:
    def alcohol_risk(x):
        ...
        if x <= 2:     return 1
        return 2
    df['음주위험군'] = df['음주빈도'].apply(alcohol_risk)
    added += ['음주위험군']
    print(f"  ✅ 음주위험군: 0=비음주/1=저위험/2=고위험")
...
if USE_WALK_LEVEL:
    def walk_level(x):
        ...
        if x <= 3:     return 1
        return 2
    df['걷기활동량'] = df['걷기일수'].apply(walk_level)
    added += ['걷기활동량']
    print(f"  ✅ 걷기활동량: 0=비활동/1=저활동/2=활동")

# 6. 근력운동 활동량
if USE_STRENGTH:
    def strength_level(x):
        if pd.isna(x): return np.nan
        if x == 0:     return 0
        if x <= 2:     return 1
        return 2
    df['근력활동량'] = df['근력운동일수'].apply(strength_level)
    added += ['근력활동량']
    print(f"  ✅ 근력활동량: 0=비활동/1=저활동/2=활동")
...
if len(candidates) > 0:
    ...
    best_thr = best_row['threshold']
    print(f"    ⚠️  Recall >= {RECALL_MIN} 만족 없음 → Recall 최대 사용: {best_thr:.2f}")
    print(f"       Recall: {best_row['recall']:.4f} | Precision: {best_row['precision']:.4f} | F1: {best_row['f1']:.4f}")
...
# ── Feature Importance ────────────────────────────────────────
print(f"[7] Feature Importance Top 20 (gain 평균, 5-fold)")
