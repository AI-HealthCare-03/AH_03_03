"""
hn22_all.sas7bdat 전처리 파이프라인 (성인 한정)
hn24 base_preprocessing.py 구조 기반 · hn22 SAS 포맷 대응
...
INPUT_PATH = os.path.join(BASE_DIR, "data", "hn22_all.sas7bdat")
OUTPUT_PATH = os.path.join(BASE_DIR, "data", "hn22_preprocessed.csv")
...
# 0. 로드 & 소아 제거
df, meta = pyreadstat.read_sas7bdat(INPUT_PATH, usecols=USE_COLS)
col_rename = {
    "sex": "성별",
    "age": "나이",
    "occp": "직업",
...
df.loc[bmi_mask, "BMI"] = (df.loc[bmi_mask, "체중"] / (df.loc[bmi_mask, "키"] / 100) ** 2).round(6)
print(f"[4] BMI {bmi_mask.sum()}건 재계산 완료 | 잔여 결측: {df['BMI'].isnull().sum()}")

# 5. 음주빈도 특수값 처리 (hn22: 8·9 추가 존재)
...
if n_bd_nan > 0:
    med_bd = df["음주빈도"].median()
    df["음주빈도"] = df["음주빈도"].fillna(med_bd)
    print(f"  → 음주빈도 9(무응답) {n_bd_nan}건 중앙값({med_bd:.1f}) 대체")
...
oe_amt = OrdinalEncoder(
    categories=[[0, 1, 2, 3, 4, 5]], handle_unknown="use_encoded_value", unknown_value=-1, encoded_missing_value=np.nan
)
