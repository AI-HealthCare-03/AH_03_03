...
async def create_faq(request: FAQCreateRequest, user: Annotated[User, Depends(require_operator_user)]):
    return await faq_service.create_faq(request)


@faq_router.patch("/faqs/{faq_id}", response_model=FAQResponse)
async def update_faq(faq_id: int, request: FAQUpdateRequest, user: Annotated[User, Depends(require_operator_user)]):
...
async def get_inquiry(inquiry_id: int, user: Annotated[User, Depends(get_request_user)]):
    ...
    ensure_owner_or_operator(inquiry.user_id, user)
    return inquiry


@faq_router.patch("/inquiries/{inquiry_id}/answer", response_model=InquiryResponse)
async def answer_inquiry(inquiry_id: int, answer: str, user: Annotated[User, Depends(require_operator_user)]):
