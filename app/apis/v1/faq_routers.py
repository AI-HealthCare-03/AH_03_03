...
async def create_faq(request: FAQCreateRequest, user: Annotated[User, Depends(require_admin_user)]):
    return await faq_service.create_faq(request)


@faq_router.patch("/faqs/{faq_id}", response_model=FAQResponse)
async def update_faq(faq_id: int, request: FAQUpdateRequest, user: Annotated[User, Depends(require_admin_user)]):
...
async def answer_inquiry(inquiry_id: int, answer: str, user: Annotated[User, Depends(require_admin_user)]):
