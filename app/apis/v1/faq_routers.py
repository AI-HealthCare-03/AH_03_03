...
from app.apis.v1.dependencies import ensure_admin_user, ensure_found, ensure_owner_or_admin
...
async def create_faq(request: FAQCreateRequest, user: Annotated[User, Depends(get_request_user)]):
    ensure_admin_user(user)
    return await faq_service.create_faq(request)


@faq_router.patch("/faqs/{faq_id}", response_model=FAQResponse)
async def update_faq(faq_id: int, request: FAQUpdateRequest, user: Annotated[User, Depends(get_request_user)]):
    ensure_admin_user(user)
...
async def get_inquiry(inquiry_id: int, user: Annotated[User, Depends(get_request_user)]):
...
async def answer_inquiry(inquiry_id: int, answer: str, user: Annotated[User, Depends(get_request_user)]):
    ensure_admin_user(user)
