...
async def get_dashboard_summary(user: Annotated[User, Depends(get_request_user_with_firebase)]):
...
async def get_dashboard_health(user: Annotated[User, Depends(get_request_user_with_firebase)]):
    return {
        "latest_health_record": await health_service.get_latest_health_record(user.id),
...
async def get_dashboard_challenges(user: Annotated[User, Depends(get_request_user_with_firebase)]):
    return {
...
async def get_dashboard_diets(user: Annotated[User, Depends(get_request_user_with_firebase)]):
...
async def get_dashboard_medications(user: Annotated[User, Depends(get_request_user_with_firebase)]):
    return {
...
async def get_dashboard_trends(user: Annotated[User, Depends(get_request_user_with_firebase)], period: str = "week"):
