...
async def create_health_record(
    request: HealthRecordCreateRequest, user: Annotated[User, Depends(get_request_user_with_firebase)]
):
...
async def get_latest_health_record(user: Annotated[User, Depends(get_request_user_with_firebase)]):
...
async def get_analysis_readiness(user: Annotated[User, Depends(get_request_user_with_firebase)]):
...
async def get_health_record(record_id: int, user: Annotated[User, Depends(get_request_user_with_firebase)]):
...
async def delete_health_record(record_id: int, user: Annotated[User, Depends(get_request_user_with_firebase)]):
    record = ensure_found(await health_service.get_health_record(record_id), "건강 기록을 찾을 수 없습니다.")
    ensure_owner(record.user_id, user)
