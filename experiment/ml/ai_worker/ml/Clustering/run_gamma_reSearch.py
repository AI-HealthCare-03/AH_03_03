...
def run_one_seed(df_base, sample_seed):
    ...
    for gamma in GAMMA_COARSE:
        ...
        print(
            f"  gamma={gamma:.3f} | Silhouette: {sil:.4f} | 최소군집비율: {min_ratio:.3f} | Cost: {cost:,.0f} | 군집크기: {sorted(counts, reverse=True)}"
        )
    ...
    best = (
        balanced.loc[balanced["silhouette"].idxmax()]
        if len(balanced) > 0
        else results_df.loc[results_df["silhouette"].idxmax()]
    )
...
def main():
    ...
    for row in summary_rows:
        print(
            f"  sample_seed={row['sample_seed']} | best_gamma={row['best_gamma']} | "
