...
def compute_metrics(predictions: list[PredictionRow]) -> dict[str, float | int]:
    ...
    predicted_food_count = sum(len(row.raw_food_names) for row in predictions)
    unmatched_food_count = sum(len(row.unmatched_food_names) for row in predictions)

    return {
        "image_count": image_count,
        "api_success_rate": _rate(sum(row.api_success for row in predictions), image_count),
        "json_parse_success_rate": _rate(sum(row.json_parse_success for row in predictions), image_count),
        "exact_match_rate_raw": _rate(sum(_is_exact_raw_match(row) for row in predictions), image_count),
        "exact_match_rate_canonical": _rate(
            sum(_is_exact_canonical_match(row) for row in predictions),
            image_count,
        ),
        "any_food_hit_rate": _rate(sum(_has_any_food_hit(row) for row in predictions), image_count),
        "unmatched_food_rate": _rate(unmatched_food_count, predicted_food_count),
        "empty_result_rate": _rate(sum(row.empty_result for row in predictions), image_count),
