...
항목명 매핑 기준:
  ...
  · hdl            → "고밀도 콜레스테롤", "HDL" 포함 행의 수치
                      ...
                      비해당 표시 시 반드시 "비해당" 반환. 비해당 행에서 0.6, 0.8 등 소수값 추출 절대 금지
  ...
  · waist_cm       → "허리둘레", "허리", "복부둘레", "복부", "WC" 포함 행의 수치
                      비해당 체크박스(□■)가 있더라도 같은 행에 숫자 수치가 기재된 경우 반드시 수치를 추출할 것
                      해당 항목이 결과지에 아예 없는 경우에만 null 반환

- 참고치(예: 200.0미만, 60.0이상, 150.0미만, 18.5~24.9, 30이상 등) 숫자는 절대 추출 금지
- 표 레이아웃이나 항목 순서가 달라도 항목명 기준으로 매칭할 것
- 항목이 비해당으로 표시된 경우, 해당 행/셀의 어떤 숫자도 추출하지 말고 반드시 "비해당" 문자열만 반환할 것
- 지질검사(total_cholesterol, triglyceride, hdl, ldl) 중 하나가 비해당이면 나머지도 비해당인 경우가 많음. 비해당 구역에서 0.6, 0.8 등 소수가 보여도 참고치 잡음이므로 추출 금지
""",
}
