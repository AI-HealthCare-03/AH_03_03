"""
ai_worker/cv/evaluate.py

GPT Vision 평가 스크립트 (방법 2 + 방법 4).

실행 방법:
    uv run python -m ai_worker.cv.evaluate
...
async def evaluate():
    ...
    image_files = sorted([f for f in IMAGES_DIR.iterdir() if f.suffix.lower() in {".jpg", ".jpeg", ".png", ".webp"}])
