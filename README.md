...
- 식단 결과의 질환별 점수 표시
- 복약 OCR 후보 추출과 confirm 흐름
- 챗봇
  - 기본: rule-based 응답
  - 옵션: OpenAI LLM rewrite
  - LangGraph 기반 상담 graph: safety bypass, keyword RAG, grounding/fallback, 추천 action node
- Langfuse trace/prompt/eval 관측 연결
- 관리자/모니터링 계정 seed
- Redis Stream 기반 async job 처리
  - AI stream: `analysis.run`, `exam_ocr.run`, `diet.analyze_image`, `medication_ocr.run`
  - Service stream: 이메일 인증, 비밀번호 재설정, 가족 초대, FCM push, 가족 알림 생성
...
- LangChain 전면 orchestration과 운영형 vector RAG 고도화
- SMS/Kakao 알림 provider
- 모델 registry/배포 자동화와 운영 모니터링 고도화
- Clova OCR 공식 실행 경로

### TODO / 현재 제한사항

- 식단 분석의 음식명 추론은 GPT Vision 옵션을 사용할 수 있지만, 식단 영양성분 데이터와 점수 산정 근거 데이터는 후속 작업에서 보강합니다.
- 현재 식단 결과 화면의 칼로리, 탄수화물, 단백질, 지방, 나트륨 등 영양성분과 질환별 식단 점수는 실제 영양 DB 고도화 전까지 시연용 더미/룰 기반 데이터로 진행 중입니다.
- 따라서 식단 점수와 영양성분은 진단 또는 정밀 영양 평가가 아니라 MVP 화면 흐름 확인용 참고값으로 봅니다.
...
│   ├── llm/                     # LangGraph chatbot, explanation, prompt, RAG runtime
│   └── jobs/                    # Redis Stream consumer, handlers, retry/DLQ, scheduler
...
├── docker-compose.yml           # legacy/minimal backend/AI 검증용 root compose
├── Makefile
└── pyproject.toml
```
...
- `docs/design/ai_feature_architecture_plan.md`: AI 기능 전체 설계 방향
- `docs/design/llm_langgraph_runtime_plan.md`: LangGraph 기반 LLM runtime, safety, RAG grounding, Langfuse trace 구조
- `docs/design/ai_worker_structure.md`: Redis Stream/ai-worker 구조
- `docs/design/cv_food_fallback_policy.md`: 식단 CV/GPT Vision fallback 정책
- `docs/deployment/ec2_prod_env.md`: EC2 + S3 데모 운영 환경 기준
...
팀원/시연 표준 실행은 `infra/docker/docker-compose.dev.yml` 기반 Full Docker dev stack입니다. 이 스택이 프론트, Nginx, FastAPI, AI Worker, PostgreSQL, Redis를 함께 실행합니다. 루트 `docker-compose.yml`은 legacy/minimal backend/AI 검증용이며, 프론트/Firebase/storage/scheduler까지 포함한 최신 dev full stack 검증에는 사용하지 않습니다.
...
컨테이너 재생성까지 포함한 표준 dev 실행은 다음 명령을 사용합니다.
...
- 루트 `docker-compose.yml`은 legacy/minimal backend/AI 검증용입니다. 프론트/Firebase/storage/scheduler까지 포함한 최신 dev full stack 검증에는 사용하지 않습니다. 프론트까지 보려면 `make demo-up` 또는 `infra/docker/docker-compose.dev.yml`을 사용합니다.
...
- Langfuse self-host 문서: `docs/ops/langfuse_self_host.md`
...
현재 Redis Stream 기반 async job은 AI stream과 service stream으로 분리해 사용합니다.

- `/api/v1/jobs/demo`
- `/api/v1/jobs/{job_id}`
- `/api/v1/analysis/run-async`
- `/api/v1/exams/{exam_id}/ocr`
- `/api/v1/diets/analyze`
- `/api/v1/medications/ocr`
- `ai-worker` service
- `ai_runtime/jobs/consumer.py`
- `ai_runtime/jobs/handlers.py`
- `ai_runtime/jobs/redis_stream.py`

주요 job type은 `analysis.run`, `exam_ocr.run`, `diet.analyze_image`, `medication_ocr.run`, `email.verification.send`, `password_reset.email.send`, `family.invite.email.send`, `fcm.push.send`, `family.notification.create`입니다.

프론트 분석 화면은 `/analysis/run-async`를 사용합니다. 기존 `/analysis/run` 동기 실행 경로는 410 Gone으로 막아 긴 작업이 요청 중 직접 실행되지 않게 합니다. OCR/식단/복약 업로드 API는 202 Accepted로 job을 만들고, 화면은 `/api/v1/jobs/{job_id}` polling으로 상태를 확인합니다.
...
운영/EC2 compose는 `infra/docker/docker-compose.prod.yml`을 기준으로 합니다.

```bash
docker compose --env-file .env -f infra/docker/docker-compose.prod.yml up -d --build --force-recreate
```

Langfuse self-host는 앱 필수 구성요소가 아니라 optional 관측 스택입니다. `infra/langfuse/docker-compose.yml`과 `infra/langfuse/.env.example`은 Langfuse 전용으로 유지합니다.
...
- 시연 체크리스트: `docs/demo/ready_checklist.md`
- 프론트 시연 체크리스트: `docs/demo/frontend_checklist.md`
- EC2/S3 데모 배포: `docs/deployment/ec2_prod_env.md`
- Langfuse self-host 운영: `docs/ops/langfuse_self_host.md`
- LLM/RAG runtime 범위: `docs/design/llm_runtime_scope.md`
- RAG runtime 책임: `docs/pipeline/rag_runtime.md`
- AI runtime 모델 범위: `docs/design/ai_worker_model_scope.md`
- ML 모델 보고서: `docs/pipeline/ml_reports/ML_total_report.md`
...
- 스키마 정규화 검토: `docs/schema_normalization_plan.md`
