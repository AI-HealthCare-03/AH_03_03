"""
02_python_ocr/test_paddle.py

PaddleOCR 2.7.3 기반 건강검진표 수치 추출 테스트
이미지(JPG) + PDF 지원

실행: python test_paddle.py
"""
    ...
    if results and results[0]:
        for line in results[0]:
            if line and len(line) >= 2:
                text = clean_line(line[1][0].strip())
                if text:
                    lines.append(text)
    return lines
...
def pdf_to_lines(file_path):
    ...
    for i, img in enumerate(images):
        ...
        lines = run_ocr_on_image_bytes(buf.getvalue())
...
def main():
    print("🔍 PaddleOCR 테스트 시작 (이미지 + PDF)")
