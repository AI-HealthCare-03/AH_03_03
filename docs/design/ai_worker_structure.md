...
현재 건강검진 OCR 공식 방향은 `EXAM_OCR_PROVIDER=auto`입니다. PDF는 측정값 페이지를 우선 선택하고 PaddleOCR을 먼저 시도하며, 실패하거나 후보가 없으면 PyMuPDF로 페이지 이미지를 만든 뒤 GPT Vision fallback을 사용할 수 있습니다. 이미지는 GPT Vision 우선, PaddleOCR fallback 정책입니다.
