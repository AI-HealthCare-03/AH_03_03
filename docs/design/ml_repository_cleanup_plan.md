...
이 문서는 ML 폴더를 서비스 추론 가능한 최소 구조로 정리하기 위한 분류 및 이동 기준 문서입니다. 실험용/legacy 파일은 삭제하지 않고 `etc/ml/` 아래로 보존 이동하며, 서비스에서 직접 import하는 최소 코드만 `ai_worker/ml/` 아래에 둡니다.
...
- 기존 노트북과 legacy 스크립트는 지식 자산이지만 서비스 import 영역에는 과합니다. 현재는 `etc/ml/` 아래로 보존 이동하고, 장기적으로 별도 archive 저장소, Notion 링크, Drive/S3, 또는 `ML-archive` repo로 이전하는 것을 권장합니다.
...
| 경로/패턴 | 권장 조치 | 이유 |
| --- | --- | --- |
| `etc/ml/ai_worker/ml/legacy/` | 보존 이동 완료 | 오래된 CatBoost/LGB/XGB 실험 코드가 많고 서비스 import 대상이 아닙니다. |
| `etc/ml/ai_worker/ml/Clustering/` | 보존 이동 완료 | 현재 서비스 분석/추론 경로와 연결되지 않은 군집 실험입니다. |
| `etc/ml/ai_worker/ml/CAT15~24/` | 보존 이동 완료 | 새 config 기반 pipeline과 중복되는 과거 학습 스크립트입니다. |
| `etc/ml/ai_worker/ml/CAT18~24/` | 보존 이동 완료 | 최종 config 근거이지만 서비스 import 영역에는 reference 이상으로 과합니다. |
| `etc/ml/ai_worker/ml/LGB15~24/` | 보존 이동 완료 | CatBoost 최종 모델 채택 후 서비스 경로에 쓰이지 않습니다. |
| `etc/ml/ai_worker/ml/LogisticR/` | 보존 이동 완료 | baseline 실험 코드입니다. 최종 서비스 추론에는 불필요합니다. |
| `etc/ml/ai_worker/ml/files/` | 보존 이동 완료 | preprocessing/EDA 자료입니다. zip 중간 산출물은 장기적으로 외부 artifact storage 권장입니다. |
| `etc/ml/ML/models/DL/*.ipynb` | 보존 이동 완료 | DL 실험 노트북입니다. |
| `etc/ml/ML/models/DM/*.ipynb` | 보존 이동 완료 | DM 실험 노트북입니다. |
| `etc/ml/ML/models/HTN/*.ipynb` | 보존 이동 완료 | HTN 실험 노트북입니다. |
| `etc/ml/ML/models/baseline/*.ipynb` | 보존 이동 완료 | baseline notebook입니다. |
| `etc/ml/ML/baseline_model/*.ipynb` | 보존 이동 완료 | baseline notebook입니다. |
| `etc/ml/ML/final(seed)/*.ipynb` | 보존 이동 완료 | seed ensemble 최종 산출물일 가능성이 있어 서비스 import 영역 밖에서 보존합니다. |
...
| `etc/ml/ai_worker/ml/db_logger.py` | legacy 내부 실험 스크립트에서만 사용 | 보존 이동 완료 |
...
- `rg -n "ai_worker.ml.legacy|ai_worker.ml.CAT|ai_worker.ml.Clustering|from ML|import ML" app ai_worker scripts docs --glob '!**/*.ipynb'`
