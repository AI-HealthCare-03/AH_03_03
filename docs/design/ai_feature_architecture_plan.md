...
- confidence가 낮거나 1000개 음식 분류 체계에 해당하지 않는 음식으로 판단되면 GPT Vision API를 fallback으로 호출한다.
...
| 자해/극단 선택/죽고 싶다 등 위기 키워드 | 챌린지 추천보다 즉시 도움 안내 및 전문기관 상담 권고 우선 |
...
| 영역 | 지표 후보 |
| --- | --- |
| CV 음식 분류 | Top-1 Accuracy, Top-5 Accuracy, F1-score, confidence |
| 질병 위험 예측 | Recall, F1-score, AUC |
| LLM/RAG | 검색 정확도, 근거 일치율, 금지 응답률, hallucination rate |
