...
- 회원가입은 아이디, 이메일, 이메일 인증, 비밀번호, 이름, 성별, 생년월일, 휴대폰 번호, 동의 항목 같은 기본정보 중심으로 유지합니다.
...
- 2026-05-23 기준 백엔드 공통 dependency 4종과 FAQ/문의/챌린지/LLM log 일부 관리자성 API 권한 체크를 1차 반영했습니다. 관리자 프론트와 audit log는 후속 구현 대상입니다.
