...
현재 제외/보류 항목은 소셜 로그인, 웨어러블 연동, 휴대폰 SMS 인증입니다. MVP/시연 회원가입 인증은 이메일 인증만 필수로 사용합니다.
...
아래 3개 항목은 풀서비스 1차 범위에서 제외하거나 보류합니다.
...
- 회원가입은 아이디, 이메일, 이메일 인증, 비밀번호, 이름, 성별, 생년월일, 동의 항목과 x1 간편 분석에 필요한 최소 건강정보 중심으로 유지합니다. 휴대폰 번호는 선택 입력 및 기존 데이터 호환용으로만 유지합니다.
...
- 휴대폰 SMS 인증은 MVP/시연 범위에서 보류합니다. 기존 `phone_number` DB 컬럼은 삭제하지 않고, 프로필/가족 기능 등 호환성용 선택값으로 유지합니다.
