...
이 실험은 운영 라우터와 연결하지 않은 독립 PoC입니다. 기존 `ai_worker/vision/ocr` 폴더는 다른 작업 영역이므로 수정하지 않고, 모든 코드는 `ai_worker/vision/clova_ocr` 내부에 분리했습니다.

## 폴더 구조

```text
ai_worker/vision/clova_ocr/
├── clova_client.py
├── extractor.py
├── pdf_converter.py
├── settings.py
...
- `ai_worker/vision/clova_ocr/data/pdfs/*`
- `ai_worker/vision/clova_ocr/data/images/*`
- `ai_worker/vision/clova_ocr/outputs/*`
- `ai_worker/vision/clova_ocr/ground_truth/*`
...
```text
ai_worker/vision/clova_ocr/outputs/{pdf_stem}/
```
...
```bash
uv run python ai_worker/vision/clova_ocr/experiments/evaluate_required_fields.py
```
...
```text
ai_worker/vision/clova_ocr/ground_truth/{pdf_stem}.json
```
...
- 기존 `ai_worker/vision/ocr` 폴더는 수정하지 않습니다.
