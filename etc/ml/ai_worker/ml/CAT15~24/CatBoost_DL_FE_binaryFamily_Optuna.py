"""
이상지질혈증유병 예측 — CatBoost + FE_binaryFamily (hn15~24 통합)
...
MODEL_DIR: str = str(BASE_DIR / "optuna_DL_FE_binaryFamily")
...
def main() -> None:
    ...
    print(f"  {'-'*60}")
    print(f"  {'원본 DL_FE (가족력 합산 0~3)':<35} {'0.7830':>7} {'0.8594':>8} {'0.6056':>7}")
    print(f"  {'금번 DL_FE_binaryFamily':<35} {test_auc:>7.4f} {test_recall:>8.4f} {test_f1:>7.4f}  ← 금번")
    print(f"\n  ΔAUC  : {test_auc - 0.7830:+.4f}")
    print(f"  ΔRecall: {test_recall - 0.8594:+.4f}")
    print(f"  ΔF1   : {test_f1 - 0.6056:+.4f}")
