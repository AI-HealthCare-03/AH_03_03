"""
DM유병 예측 — CatBoost + FE_ordinalFamily (hn15~24 통합)
...
MODEL_DIR: str = str(BASE_DIR / "optuna_DM_FE_ordinalFamily")
...
def main() -> None:
    ...
    print(f"  {'-'*63}")
    print(f"  {'원본 DM_FE (가족력 합산 0~3)':<38} {'0.8152':>7} {'0.8528':>8} {'0.6009':>7}")
    print(f"  {'binary DM_FE_binaryFamily':<38} {'0.8043':>7} {'0.8485':>8} {'0.5446':>7}")
    print(f"  {'ordinal DM_FE_ordinalFamily':<38} {test_auc:>7.4f} {test_recall:>8.4f} {test_f1:>7.4f}  ← 금번")
    print(f"\n  vs 원본  ΔAUC: {test_auc-0.8152:+.4f} | ΔRecall: {test_recall-0.8528:+.4f} | ΔF1: {test_f1-0.6009:+.4f}")
    print(f"  vs binary ΔAUC: {test_auc-0.8043:+.4f} | ΔRecall: {test_recall-0.8485:+.4f} | ΔF1: {test_f1-0.5446:+.4f}")
