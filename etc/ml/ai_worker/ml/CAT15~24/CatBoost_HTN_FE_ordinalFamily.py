"""
고혈압유병 예측 — CatBoost + FE_ordinalFamily (hn15~24 통합)
...
MODEL_DIR: str = str(BASE_DIR / "optuna_HTN_FE_ordinalFamily")
...
def main() -> None:
    ...
    print(f"  {'-'*63}")
    print(f"  {'원본 HTN_FE (가족력 합산 0~3)':<38} {'0.8587':>7} {'0.8592':>8} {'0.6862':>7}")
    print(f"  {'binary HTN_FE_binaryFamily':<38} {'0.8567':>7} {'0.8622':>8} {'0.6852':>7}")
    print(f"  {'ordinal HTN_FE_ordinalFamily':<38} {test_auc:>7.4f} {test_recall:>8.4f} {test_f1:>7.4f}  ← 금번")
    print(f"\n  vs 원본  ΔAUC: {test_auc-0.8587:+.4f} | ΔRecall: {test_recall-0.8592:+.4f} | ΔF1: {test_f1-0.6862:+.4f}")
    print(f"  vs binary ΔAUC: {test_auc-0.8567:+.4f} | ΔRecall: {test_recall-0.8622:+.4f} | ΔF1: {test_f1-0.6852:+.4f}")
