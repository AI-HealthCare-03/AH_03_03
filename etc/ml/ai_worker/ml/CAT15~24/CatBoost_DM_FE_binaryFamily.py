"""
당뇨유병 예측 — CatBoost + FE_binaryFamily (hn15~24 통합)
...
변경사항:
  ...
  - Optuna 스킵 → optuna_DM_FE/best_params.json 고정 사용
  - FE 조합: 나이구간 + BMI구간 + 가족력binary + BMI_X_나이
"""
...
PARAMS_PATH: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT15~24/outputs/optuna_DM_FE/best_params.json"
MODEL_DIR: str = str(BASE_DIR / "optuna_DM_FE_binaryFamily")
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if USE_FAMILY_BINARY:
        for prefix in ["고혈압가족력", "당뇨가족력", "고지혈증가족력"]:
            ...
            if existing:
                df[prefix] = df[existing].max(axis=1)
                added.append(prefix)
...
def main() -> None:
    ...
    print(f"\n[3] Optuna 스킵 — optuna_DM_FE/best_params.json 고정 사용")
    ...
    print(f"  {'-'*60}")
    print(f"  {'원본 DM_FE (가족력 합산 0~3)':<35} {'0.8152':>7} {'0.8528':>8} {'0.6009':>7}")
    print(f"  {'금번 DM_FE_binaryFamily':<35} {test_auc:>7.4f} {test_recall:>8.4f} {test_f1:>7.4f}  ← 금번")
    print(f"\n  ΔAUC  : {test_auc - 0.8152:+.4f}")
    print(f"  ΔRecall: {test_recall - 0.8528:+.4f}")
    print(f"  ΔF1   : {test_f1 - 0.6009:+.4f}")
