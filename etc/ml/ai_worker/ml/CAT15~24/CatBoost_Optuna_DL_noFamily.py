"""
이상지질혈증유병 예측 — CatBoost + Optuna + FE (hn15~24 통합)
...
MODEL_DIR: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT15~24/outputs/optuna_DL_FE_noFamily"
...
USE_FAMILY_SUM: bool = False  # 실험 2: 가족력 제거
...
def main() -> None:
    ...
    best_params = {
        "iterations":          254,
        "learning_rate":       0.04853762811349759,
        "depth":               5,
        "l2_leaf_reg":         5.682065189130785,
        "bagging_temperature": 0.8766012324747308,
        "random_strength":     0.5901197168505028,
    }
    ...
    # ── 원본 모델과 비교 요약 ─────────────────────────────────
    print("\n" + "=" * 60)
    print("[비교] 가족력 유/무 성능 비교 (이상지질혈증유병)")
    print("=" * 60)
    print(f"  {'구분':<30} {'AUC':>7} {'Recall':>8} {'F1':>7}")
    print(f"  {'-'*55}")
    print(f"  {'원본 (가족력 포함, HN 내부검증)':<30} {'0.7830':>7} {'0.8594':>8} {'0.6056':>7}")
    print(f"  {'실험2 (가족력 제거, HN 내부검증)':<30} {test_auc:>7.4f} {test_recall:>8.4f} {test_f1:>7.4f}  ← 금번")
    print(f"\n  ※ AUC 차이    : {test_auc - 0.7830:+.4f}")
    print(f"     Recall 차이 : {test_recall - 0.8594:+.4f}")
    print(f"     F1 차이     : {test_f1 - 0.6056:+.4f}")
    print("\n  → 차이가 크면: 가족력이 주요 원인")
    print("  → 차이가 작으면: 가족력 외 다른 원인 탐색 필요")
