"""
고혈압유병 예측 — CatBoost + FE_binaryFamily (hn15~24 통합)
Python 3.13 | catboost>=1.2 | scikit-learn>=1.4

변경사항:
  - 가족력 합산(0~3) → binary(0/1/NaN) 3개로 재설계
    · HN 원데이터: 부/모/형제 중 하나라도 1 → 1, 모두 0 → 0, 모두 NaN → NaN
    · 서비스 입력: YES=1, NO=0, 모름=NaN 과 분포 일치
  - Optuna 스킵 → optuna_HTN_FE/best_params.json 고정 사용
  - FE 조합: 나이구간 + 음주위험군 + 걷기활동량 + 가족력binary + BMI_X_나이
"""
...
PARAMS_PATH: str = "/Users/admin/PycharmProjects/AH_03_03/ai_worker/ml/CAT15~24/outputs/optuna_HTN_FE/best_params.json"
MODEL_DIR: str = str(BASE_DIR / "optuna_HTN_FE_binaryFamily")
...
USE_FAMILY_BINARY: bool = True   # 합산 대신 binary
...
def apply_feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if USE_FAMILY_BINARY:
        # 부/모/형제 중 하나라도 1이면 1, 모두 0이면 0, 모두 NaN이면 NaN
        # → 서비스 입력(YES=1/NO=0/모름=NaN)과 분포 일치
        for prefix in ["고혈압가족력", "당뇨가족력", "고지혈증가족력"]:
            cols = [f"{prefix}_부", f"{prefix}_모", f"{prefix}_형제"]
            existing = [c for c in cols if c in df.columns]
            if existing:
                df[prefix] = df[existing].max(axis=1)  # NaN은 max에서 자동 전파
                added.append(prefix)
        print(f"  [ON] 가족력 binary: {[p for p in ['고혈압가족력','당뇨가족력','고지혈증가족력'] if p in df.columns]}")
    ...
    # 원천 부/모/형제 컬럼 제거 (binary로 대체됐으므로)
    granular_cols = [
...
def main() -> None:
    ...
    print(f"\n[3] Optuna 스킵 — optuna_HTN_FE/best_params.json 고정 사용")
    ...
    # ── 기존 모델과 비교 요약 ─────────────────────────────────
    print("\n" + "=" * 60)
    print("[비교] 가족력 합산 vs binary 성능 비교")
    print("=" * 60)
    print(f"  {'구분':<35} {'AUC':>7} {'Recall':>8} {'F1':>7}")
    print(f"  {'-'*60}")
    print(f"  {'원본 HTN_FE (가족력 합산 0~3)':<35} {'0.8587':>7} {'0.8592':>8} {'0.6862':>7}")
    print(f"  {'금번 HTN_FE_binaryFamily':<35} {test_auc:>7.4f} {test_recall:>8.4f} {test_f1:>7.4f}  ← 금번")
    print(f"\n  ΔAUC  : {test_auc - 0.8587:+.4f}")
    print(f"  ΔRecall: {test_recall - 0.8592:+.4f}")
    print(f"  ΔF1   : {test_f1 - 0.6862:+.4f}")
