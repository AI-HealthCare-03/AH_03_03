...
OOF_DIR = os.path.join(BASE_DIR, "outputs", "tuned_catboost_HL_100")
OUTPUT_DIR = os.path.join(BASE_DIR, "outputs", "threshold_tuning_HL")
...
axes[0].set_title("Recall / Precision / F1 vs Threshold (이상지질혈증)")
axes[0].legend()
...
axes[1].set_title("FN / FP vs Threshold (이상지질혈증)")
axes[1].legend()
...
).to_csv(os.path.join(OUTPUT_DIR, "best_threshold.csv"), index=False)
