...
OUT_DIR = os.path.join(BASE_DIR, "ml", "NHIS", "outputs", "Basic_EDA")
...
colors = miss_df["결측률(%)"].apply(lambda x: "#E24B4A" if x >= 50 else ("#EF9F27" if x >= 1 else "#1D9E75"))
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "eda_01_missing.png"), dpi=150, bbox_inches="tight")
plt.close()
print("\n저장: eda_01_missing.png")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "eda_02_demographic.png"), dpi=150, bbox_inches="tight")
plt.close()
print("저장: eda_02_demographic.png")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "eda_03_clinical_dist.png"), dpi=150, bbox_inches="tight")
plt.close()
print("저장: eda_03_clinical_dist.png")
...
axes[2].bar([prot_labels.get(k, k) for k in prot_cnt.index], prot_cnt.values, color="#534AB7", edgecolor="white")
axes[2].set_title("요단백")
axes[2].yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}"))

plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "eda_04_lifestyle.png"), dpi=150, bbox_inches="tight")
plt.close()
print("저장: eda_04_lifestyle.png")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "eda_05_feature_corr.png"), dpi=150, bbox_inches="tight")
plt.close()
print("저장: eda_05_feature_corr.png")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "eda_06_age_gender_box.png"), dpi=150, bbox_inches="tight")
plt.close()
print("저장: eda_06_age_gender_box.png")

print("\n=== 모든 EDA 완료 ===")
print("생성 파일:")
for i, name in enumerate(
    [
        "eda_01_missing.png         — 결측률 현황",
        "eda_02_demographic.png     — 성별·연령대 분포",
        "eda_03_clinical_dist.png   — 임상수치 히스토그램",
        "eda_04_lifestyle.png       — 생활습관 분포",
        "eda_05_feature_corr.png    — 수치형 피처 상관계수",
        "eda_06_age_gender_box.png  — 성별×연령대 박스플롯",
    ],
    1,
):
    print(f"  {i}. {name}")
