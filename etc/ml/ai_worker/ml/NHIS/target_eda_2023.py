...
OUT_DIR = os.path.join(BASE_DIR, "ml", "NHIS", "outputs", "Target_EDA_2023")
...
for t, l in zip(targets, labels):
    ...
    print(f"[{l:8s}] 양성: {pos:>8,} ({pos / len(valid) * 100:.1f}%)  음성: {neg:>8,}  결측: {na:,}")
...
fig.suptitle("타겟 변수 중복도 EDA — 국민건강보험공단 건강검진 2023", fontsize=14)
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "target_overlap_eda_2023.png"), dpi=150, bbox_inches="tight")
plt.show()
print("\n시각화 저장 완료: target_overlap_eda_2023.png")

# ────────────────────────────────────────────
# 7. 타겟 포함 데이터 저장 (선택)
# ────────────────────────────────────────────
# df.to_csv(os.path.join(OUT_DIR, "health_with_targets_2023.csv"), index=False, encoding="utf-8-sig")
