...
def make_targets(df):
    ...
    # 고혈압
    df["target_hypertension"] = np.where((df["수축기혈압"] >= 140) | (df["이완기혈압"] >= 90), 1, 0)
    df.loc[df["수축기혈압"].isna() & df["이완기혈압"].isna(), "target_hypertension"] = np.nan

    # 이상지질혈증
    hdl_low = ((df["성별코드"] == 1) & (df["HDL콜레스테롤"] < 40)) | (
        (df["성별코드"] == 2) & (df["HDL콜레스테롤"] < 50)
    )
    dyslipidemia = (
        (df["총콜레스테롤"] >= 200) | (df["LDL콜레스테롤"] >= 130) | (df["트리글리세라이드"] >= 150) | hdl_low
    )
    ...
    ms_score = (
        abdom.astype(float) + tg_hi.astype(float) + hdl_low.astype(float) + bp_ms.astype(float) + gluc_ms.astype(float)
    )
    ...
    ggt_high = ((df["성별코드"] == 1) & (df["감마지티피"] > 63)) | ((df["성별코드"] == 2) & (df["감마지티피"] > 35))
...
def make_features(df):
    ...
    df["bmi_category"] = pd.cut(
        df["bmi"], bins=[0, 18.5, 23.0, 25.0, 30.0, 35.0, 999], labels=[0, 1, 2, 3, 4, 5], right=False
    ).astype(float)
    ...
    le = LabelEncoder()
    df["gender_age_enc"] = le.fit_transform(df["성별코드"].astype(str) + "_" + df["연령대코드(5세단위)"].astype(str))
...
BEST_PARAMS_B = {
    "당뇨위험": {
        "LightGBM": dict(
            n_estimators=522,
            learning_rate=0.05034719040252834,
            num_leaves=101,
            max_depth=8,
            min_child_samples=81,
            subsample=0.5420498720757686,
            colsample_bytree=0.9656316260251595,
            reg_alpha=0.7639368798428724,
            reg_lambda=0.00618006575315154,
        ),
        "CatBoost": dict(
            iterations=806,
            learning_rate=0.04623826630220252,
            depth=10,
            l2_leaf_reg=1e-3,  # degenerate 방지: 원본 1.86e-5 → 1e-3 상향
            bagging_temperature=0.09197660588086623,
            random_strength=0.0005944485244359853,
        ),
    },
    "고혈압": {
        "LightGBM": dict(
            n_estimators=507,
            learning_rate=0.22331242412867336,
            num_leaves=149,
            max_depth=12,
            min_child_samples=10,
            subsample=0.7954172753805078,
            colsample_bytree=0.7247644266894295,
            reg_alpha=3.831e-05,
            reg_lambda=0.0006679438865010269,
        ),
        "CatBoost": dict(
            iterations=742,
            learning_rate=0.2849015462477188,
            depth=10,
            l2_leaf_reg=1e-3,  # degenerate 방지: 원본 1.56e-6 → 1e-3 상향
            bagging_temperature=0.776653017329181,
            random_strength=1.580e-05,
        ),
    },
    "이상지질혈증": {
        "LightGBM": dict(
            n_estimators=376,
            learning_rate=0.09997948419313435,
            num_leaves=35,
            max_depth=12,
            min_child_samples=22,
            subsample=0.8873235866452984,
            colsample_bytree=0.7962071632765588,
            reg_alpha=3.361e-08,
            reg_lambda=1.159e-06,
        ),
        "CatBoost": dict(
            iterations=814,
            learning_rate=0.06492060953012901,
            depth=10,
            l2_leaf_reg=1e-3,  # degenerate 방지: 원본 3.75e-5 → 1e-3 상향
            bagging_temperature=0.29206227335309387,
            random_strength=5.620e-06,
        ),
    },
    "대사증후군": {
        "LightGBM": dict(
            n_estimators=902,
            learning_rate=0.09685139354186116,
            num_leaves=139,
            max_depth=12,
            min_child_samples=53,
            subsample=0.649352863649425,
            colsample_bytree=0.7727081638649321,
            reg_alpha=1.417e-06,
            reg_lambda=0.0009399152029515449,
        ),
        "CatBoost": dict(
            iterations=922,
            learning_rate=0.13844752134393362,
            depth=10,
            l2_leaf_reg=0.00723482443317559,
            bagging_temperature=0.708032370100854,
            random_strength=2.185e-08,
        ),
    },
    "간기능이상": {
        "LightGBM": dict(
            n_estimators=985,
            learning_rate=0.09792171704582421,
            num_leaves=137,
            max_depth=12,
            min_child_samples=20,
            subsample=0.8011011032029123,
            colsample_bytree=0.9633869918248013,
            reg_alpha=5.296e-05,
            reg_lambda=0.6975799435019739,
        ),
        "CatBoost": dict(
            iterations=934,
            learning_rate=0.09330545961114106,
            depth=10,
            l2_leaf_reg=0.018218240559437857,
            bagging_temperature=0.5525877889462425,
            random_strength=0.7494583751599895,
        ),
    },
    "신장단백뇨": {
        "LightGBM": dict(
            n_estimators=957,
            learning_rate=0.23146375268766253,
            num_leaves=109,
            max_depth=10,
            min_child_samples=52,
            subsample=0.857210459075525,
            colsample_bytree=0.8043497429670368,
            reg_alpha=3.892069681051425,
            reg_lambda=0.05620347256425568,
        ),
        "CatBoost": dict(
            iterations=878,
            learning_rate=0.01596021282510282,
            depth=8,
            l2_leaf_reg=1e-3,  # degenerate 방지: 원본 3.14e-7 → 1e-3 상향
            bagging_temperature=0.8232527440752893,
            random_strength=3.514e-05,
        ),
    },
}
...
def get_models(scale_pos_weight=1.0, class_weight=None, target_name=None, imb_ensemble=False):
    ...
    if target_name and target_name in BEST_PARAMS_B:
        ...
        lgbm_params = dict(
            n_estimators=500, learning_rate=0.05, num_leaves=63, random_state=RANDOM_STATE, n_jobs=-1, verbose=-1
        )
        catb_params = dict(iterations=500, learning_rate=0.05, depth=6, random_state=RANDOM_STATE, verbose=0)
    ...
    models = {
        "LR": Pipeline(
            [
                ...
                ("model", LogisticRegression(max_iter=1000, random_state=RANDOM_STATE, n_jobs=-1, class_weight=cw)),
            ]
        ),
        "RF": Pipeline(
            [
                ...
                (
                    ...
                    RandomForestClassifier(
                        n_estimators=300, max_depth=10, random_state=RANDOM_STATE, n_jobs=-1, class_weight=cw
                    ),
                ),
            ]
        ),
        ...
        "CatBoost": CatBoostClassifier(**catb_params, auto_class_weights=catb_auto),
    }
    ...
    if imb_ensemble:
        models["EasyEnsemble"] = Pipeline(
            [
                ("imputer", KNNImputer(n_neighbors=5)),
                ("model", EasyEnsembleClassifier(n_estimators=10, random_state=RANDOM_STATE, n_jobs=-1)),
            ]
        )
...
df24 = make_targets(df24)
df24 = make_features(df24)
print(f"  완료: {df24.shape[0]:,}행")

print("2023 데이터 로드 중...")
df23 = pd.read_csv(DATA_23, encoding="cp949")
df23 = preprocess(df23)
df23 = make_targets(df23)
df23 = make_features(df23)
print(f"  완료: {df23.shape[0]:,}행")

print("2022 데이터 로드 중...")
df22 = pd.read_csv(DATA_22, encoding="cp949")
df22 = df22.rename(columns={"성별": "성별코드"})
df22 = preprocess(df22)
df22 = make_targets(df22)
df22 = make_features(df22)
...
for exp in EXPERIMENTS:
    ...
    for target_name, target_col in TARGETS.items():
        ...
        if use_jitter:
            ...
            X_train["obesity_combined"] = ((X_train["bmi"] >= 25) & (X_train["waist_height_ratio"] >= 0.5)).astype(
                float
            )
        ...
        for model_name, model in get_models(
            target_name=target_name, class_weight=class_weight, imb_ensemble=imb_ensemble
        ).items():
            ...
            print(
                f"  {model_name:<12} AUC: {auc:.4f} | Recall: {recall:.4f} {recall_ok} | "
            ...
            result = {
                ...
                "TN": int(tn),
                "FN": int(fn),
                "Recall_달성": recall >= RECALL_TARGET,
                "F1_달성": f1 >= 0.6,
            }
        ...
        if use_optuna:
            for model_name in tune_models:
                ...
                if model_name == "LightGBM":
                    ...
                    best_model = LGBMClassifier(**best_params, random_state=RANDOM_STATE, n_jobs=-1, verbose=-1)
                ...
                f1_ok = "✅" if f1 >= 0.6 else "❌"
                print(
                    f"  {model_name + '(Optuna)':<18} AUC: {auc:.4f} | Recall: {recall:.4f} {recall_ok} | "
                    f"F1: {f1:.4f} {f1_ok} | Precision: {prec:.4f} | Threshold: {best_thr:.2f}"
                )
        ...
        if oof_ensemble:
            ...
            if len(oof_probs) >= 2:
                ...
                print(
                    f"  {'OOF_Ensemble':<18} AUC: {auc_ens:.4f} | Recall: {recall_ens:.4f} {recall_ok} | "
                ...
                ens_result = {
                    ...
                    "TN": int(tn),
                    "FN": int(fn),
                    "Recall_달성": recall_ens >= RECALL_TARGET,
                    "F1_달성": f1_ens >= 0.6,
                }
        ...
        if stacking:
            ...
            print(
                f"  {'Stacking':<18} AUC: {auc_st:.4f} | Recall: {recall_st:.4f} {recall_ok} | "
            ...
            stack_result = {
                ...
                "모델": "Stacking",
                "AUC": round(auc_st, 4),
                "Recall": round(recall_st, 4),
                "F1": round(f1_st, 4),
                "Precision": round(prec_st, 4),
                "Threshold": thr_st,
                "TP": int(tp),
                "FP": int(fp),
                "TN": int(tn),
                "FN": int(fn),
                "Recall_달성": recall_st >= RECALL_TARGET,
                "F1_달성": f1_st >= 0.6,
            }
...
best_df = (
    results_df.sort_values(["실험", "타겟", "Recall_달성", "F1"], ascending=[True, True, False, False])
    .groupby(["실험", "타겟"])
    .first()
    .reset_index()
)
