...
# [4] 고지혈증
hdl_low = ((df["성별코드"] == 1) & (df["HDL콜레스테롤"] < 40)) | ((df["성별코드"] == 2) & (df["HDL콜레스테롤"] < 50))
dyslipidemia = (df["총콜레스테롤"] >= 200) | (df["LDL콜레스테롤"] >= 130) | (df["트리글리세라이드"] >= 150) | hdl_low
...
targets = ["target_diabetes", "target_hypertension", "target_dyslipidemia", "target_metabolic", "target_liver"]
...
df_x1["bmi_category"] = pd.cut(df_x1["bmi"], bins=bmi_bins, labels=bmi_labels, right=False).astype(float)
...
age_mid = {5: 27, 6: 32, 7: 37, 8: 42, 9: 47, 10: 52, 11: 57, 12: 62, 13: 67, 14: 72, 15: 77, 16: 82, 17: 87, 18: 92}
...
df_x1["obesity_combined"] = ((df_x1["bmi"] >= 25) & (df_x1["waist_height_ratio"] >= 0.5)).astype(float)
...
print(missing_x1)
print(
    f"결측 있는 행: {df_x1[x1_raw].isnull().any(axis=1).sum():,}건 "
