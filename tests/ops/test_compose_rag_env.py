...
def test_dev_compose_passes_rag_embedding_env_to_app_containers() -> None:
    ...
    assert compose.count("RAG_RETRIEVAL_STRATEGY:") >= 2
    assert compose.count("MAIN_CHATBOT_RAG_STRATEGY:") >= 2
    assert compose.count("ANALYSIS_EXPLANATION_LLM_REWRITE_ENABLED:") >= 2
...
def test_compose_rag_defaults_stay_safe() -> None:
    for compose_path in (DEV_COMPOSE, PROD_COMPOSE):
        ...
        assert "RAG_RETRIEVAL_STRATEGY: ${RAG_RETRIEVAL_STRATEGY:-keyword_only}" in compose
        assert "MAIN_CHATBOT_RAG_STRATEGY: ${MAIN_CHATBOT_RAG_STRATEGY:-keyword_only}" in compose
        assert (
            "ANALYSIS_EXPLANATION_LLM_REWRITE_ENABLED: ${ANALYSIS_EXPLANATION_LLM_REWRITE_ENABLED:-false}"
        ) in compose
