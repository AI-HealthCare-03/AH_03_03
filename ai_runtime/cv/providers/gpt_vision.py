...
- 조리법이나 음식 종류가 시각적으로 분명하면 음식명에 포함하세요.
  예: 볶음밥, 국물 면요리, 중식 면요리, 생선구이, 닭고기 볶음, 샐러드
- 음식, 음료, 과일, 채소, 원재료, 간식, 소스, 피클, 샐러드, 포장 식품은 모두 유효한 음식 후보입니다.
- 정확한 음식명을 모르더라도 foods를 비우지 말고, 보이는 범위에서 더 넓은 후보를 낮은 confidence로 반환하세요.
  예: "중식 면요리", "볶음밥", "국물 면요리", "채소 샐러드", "튀김류"
- 낮은 확신은 빈 배열이 아니라 낮은 confidence 값으로 표현하세요.
- foods=[]는 음식, 음료, 식재료, 소스, 포장 식품이 전혀 보이지 않는 경우에만 사용하세요.
